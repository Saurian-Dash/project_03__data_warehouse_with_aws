...
class PostgreSQLOperator:
    ...
    def create_connection(self, endpoint, autocommit=False):
        """
        Create connection to database endpoint with specified auto-commit settings.
        Upon connection, a cursor and connection object are attached as properties
        to this class.

        Args:
            endpoint (string): Endpoint of Redshift cluster to be accessed by the
            application. The cluster endpoint is attached to the Redshift client
            upon successful creation of a cluster.
    ...
    def execute_query(self, query, *args):
        ...
        Args:
            ...
            string formatted SQL query. This application favours parametrised SQL
            queries with keyword arguments, thus this argument is not used.
    ...
    def get_tables(self):
        """
        Execute a SQL query to return a list of all tables in the data warehouse.
    ...
    def execute_tasks(self, manifest):
        """
        Execute a set of commands contained within a task in a manifest. A task is
        a dictionary containing the details of a database operation, this application
        uses these to carry out operations, such as creating tables or transforming
        data before loading to the dimensional model.

        Args:
            manifest (list): A manifest is a list, of dictionary objects containing
    ...
    def drop_tables(self):
        """
        Iterate over all data warehouse tables and execute a DROP TABLE SQL query.
    ...
    def copy_s3_data(self, manifest, role_arn):
        ...
        Args:
            manifest (list): This application uses manifests, a manifest is a list,
            of dictionary objects containing the details of a task. These keyword
            arguments are passed to the task's parametrised SQL query which this
            function executes.

            role_arn: (string): The IAM role arn which enables the Redshift cluster
            to read from S3, this is available as a property of the IAMOperator.
    ...
    def close_connection(self):
        """
        Close current database session. This function is invoked at the end of the
        ETL operation.
